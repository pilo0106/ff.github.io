		...
		<div class="main">
			<div class="recordposition">
				<div class="record">
					...
					<div class="tab">
						<div class="tab_pst1">
							<button class="tablinks" onclick="openCity(event, 'choose1')">Loan</button>
							<button class="tablinks" onclick="openCity(event, 'choose2')">Overdraft</button>
							<button class="tablinks" onclick="openCity(event, 'check')">Unconfirmed<?php echo ' '.'<span style="color:rgb(255, 133, 67); font-size: 14px;font-weight:bold;">'.$qqnum.'</span>'; ?></button>
							<button class="tablinks" onclick="openCity(event, 'finish')"> Completed</button>
						</div>
						<div class="tab_pst2">
							<button class="tablinks" onclick="openCity(event, 'newloan')">New Loan Record</button>
							<button class="tablinks" onclick="openCity(event, 'newOutstanding')">New Overdraft Record</button>
						</div>
					</div>
						...
						<div id="choose1" class="tabcontent">
							<table>
								...
								<tr>
									...
									<th>Notes</th>
									<th>Added date</th>
									<th>&nbsp&nbsp Deb-state &nbsp&nbsp</th>
									<th>&nbsp&nbsp Cred-state &nbsp&nbsp</th>
								</tr>
								...
								<tr>
									...
									<?php   
										if($row['ct_state'] == "未還" && $row['creditor']==$username ) {	
											...
											<td><form action="en_change_state_loan.php" method="post">
								...
								<tr>
									...
									<?php   
										if($row['dt_state'] == "未還" && $row['debtor']==$username ) {	
											?>
											<td><form action="en_change_state_o.php" method="post">
													<input type="hidden" name="hidden_id" value=<?php echo $row['id']; ?>>	
								...
								<tr>
									...
									<?php
										...
										{	
											?>
											<td class="loan_cdt_color"><?php echo $row['dt_state']; ?></td>
											<td><form action="en_change_state_ck.php" method="post">
													<input type="hidden" name="hidden_id" value=<?php echo $row['id']; ?>>	
													<input class="changebtn" type="submit" name="send" value=<?php echo $row['ct_state']; ?>>
												</form>
											</td>
					...
					<div class="page">	
						...
						<table>
								<?php
									$query = "SELECT * FROM record WHERE creditor='$username' and ct_state='已完成' or debtor= '$username' and ct_state='已完成'"; //搜尋 *(全部欄位) ，從 表
									$query_run = mysqli_query($conn,$query); //$conn <<此變數來自引入的 db_cn.php
					...
					<div class="page">
						...
						<div id="newloan" class="tabcontent">
							...
							<form action="en_addloanrecord.php" method="post">  <!--連結php-->
								...
								<div class="inputContainer1">
									...
									<input name="mny" type="text" class="input" size="15" placeholder="" required>
								</div>
								<div class="inputContainer">
									...
									<input name="nt" type="text" class="input2" size="15" placeholder="備註">
					...
					<div class="page">
						...
						<div id="newOutstanding" class="tabcontent">
							...
							<form action="en_addoutstandingrecord.php" method="post">  <!--連結php-->
								<div class="inputContainer2">
									<select name="dt">
										<?php
											if(mysqli_num_rows($query_run) > 0)
